...
To determine, whether all features are independant from each other, their correlations are calculated, which is shown in figure \ref{fig:corr} . Here the correlation
...
Most features show only small positive correlations with each other. The most noticeable exceptions are the cluster positions, which correlate particulary high within each
triplet. All charge deposition features and the $\chi^2$ value show almost no correlation with any other feature, though this does not necessarily mean that they are
...
To prevent too many splits on less useful features, the cluster positions on the six sensors in horizontal and vertical direction are not given as input to the machine learner,
...
\begin{figure}
  ...
  \vspace{-1.8cm}
  \caption{Pearson correlation coefficient of the kink angles, the cluster position, the charge deposition, and the $\chi^2$ value.}
  \label{fig:corr}
\end{figure}
